module main::omni_cache {
    ...
    fun init_module(account: &signer) {
        ...
        let special_events =
            SpecialEvents { special_events_table: special_events_table };
    ...
    public(friend) entry fun unlock_cache_via_event(account: &signer) acquires OmniCacheData, SpecialEquipmentCacheData, NormalEquipmentCacheData, SpecialEventsInfoEntry {
        ...
        simple_map::upsert(
            &mut special_events_info_entry.whitelist_map,
            account_addr,
            amount_available_to_mint - 1,
        );

        random_mint(account);
    }

    public entry fun add_equipment_to_cache(
        account: &signer, cache_id: u64, equipment_id: u64
        ...
        if (cache_id == 0) {
            ...
            let normal_equipment_cache_table_length: u64 = aptos_std::smart_table::length(
                normal_equipment_cache_table
            );
            smart_table::add(
                normal_equipment_cache_table,
                normal_equipment_cache_table_length,
                equipment_id,
            );

        } else if (cache_id == 1) {
            let special_equipment_cache_table = &mut borrow_global_mut<
                SpecialEquipmentCacheData>(@main).table;
            let special_equipment_cache_table_length: u64 = aptos_std::smart_table::length(
                special_equipment_cache_table
            );
            smart_table::add(
                special_equipment_cache_table,
                special_equipment_cache_table_length,
                equipment_id,
            );
        }
    }

    public entry fun modify_special_event_struct(
        account: &signer, name: String, start_time: u64, end_time: u64
    ...
    public entry fun upsert_whitelist_address(
        account: &signer, modify_address: address, amount: u64
    ) acquires SpecialEventsInfoEntry {
        let account_addr = signer::address_of(account);
        admin::assert_is_admin(account_addr);
        let special_events_info_entry = borrow_global_mut<SpecialEventsInfoEntry>(@main);
        simple_map::upsert(
            &mut special_events_info_entry.whitelist_map, modify_address, amount
        );
    }

    public entry fun add_whitelist_addresses(
        account: &signer, address_vector: vector<address>, amount_vector: vector<u64>
    ) acquires SpecialEventsInfoEntry {
        let account_addr = signer::address_of(account);
        admin::assert_is_admin(account_addr);
        assert!(
    ...
    public entry fun reset_event_and_add_addresses(
        account: &signer,
        name: String,
        start_time: u64,
        end_time: u64,
    ...
    public entry fun reset_cache_and_add_equipment_ids(
        account: &signer,
        cache_id: u64,
        row_id_vector: vector<u64>,
        ...
        if (cache_id == 0) {
            ...
            smart_table::add_all(
                normal_equipment_cache_table, row_id_vector, equipment_id_vector
            );
        } else if (cache_id == 1) {
            let special_equipment_cache_table = &mut borrow_global_mut<
                SpecialEquipmentCacheData>(@main).table;
            ...
            smart_table::add_all(
                special_equipment_cache_table, row_id_vector, equipment_id_vector
            );
        }
    }

    public entry fun modify_omni_cache_data(
        account: &signer,
        shards_to_unlock_cache: u64,
        normal_equipment_weight: u64,
        special_equipment_weight: u64
    ) acquires OmniCacheData {
        let account_addr = signer::address_of(account);
        admin::assert_is_admin(account_addr);
    ...
    fun random_mint(account: &signer) acquires OmniCacheData, SpecialEquipmentCacheData, NormalEquipmentCacheData {
        ...
        let random_number =
            randomness::u64_range(
                1, normal_equipment_weight + special_equipment_weight + 1
            );
        if (random_number <= normal_equipment_weight) {
            ...
            let random_equipment_id: u64 = *smart_table::borrow(
                normal_equipment_cache_table, row_id
            );
            equipment::mint_equipment(account, random_equipment_id);
        } else {
            let special_equipment_cache_table = &borrow_global<SpecialEquipmentCacheData>(
                @main
            ).table;
            let special_equipment_cache_table_length: u64 = aptos_std::smart_table::length(
                special_equipment_cache_table
            ...
            let random_equipment_id: u64 = *smart_table::borrow(
                special_equipment_cache_table, row_id
            );
            equipment::mint_equipment(account, random_equipment_id);
        }
    }
    ...
    public fun get_special_event_struct_details(): (String, u64, u64) acquires SpecialEventsInfoEntry {
        let special_events_info_entry = borrow_global<SpecialEventsInfoEntry>(@main);
        (
            special_events_info_entry.name,
            special_events_info_entry.start_time,
            special_events_info_entry.end_time
        )
    }
    ...
    public fun get_omni_cache_data(): (u64, u64, u64) acquires OmniCacheData {
        let omni_cache_data = borrow_global<OmniCacheData>(@main);
        (
            omni_cache_data.shards_to_unlock_cache,
            omni_cache_data.normal_equipment_weight,
            omni_cache_data.special_equipment_weight
        )
    }
