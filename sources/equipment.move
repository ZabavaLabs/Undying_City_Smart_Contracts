module main::equipment {
    ...
    fun init_module(account: &signer) {
        let (signer_resource, token_signer_cap) =
            account::create_resource_account(account, APP_SIGNER_CAPABILITY_SEED);
        let (_, burn_signer_capability) =
            account::create_resource_account(account, BURN_SIGNER_CAPABILITY_SEED);
        ...
        let collection_signer =
            create_equipment_collection(
                &signer_resource, description, name, uri
            );

        move_to(
            &collection_signer,
            ResourceCapability { capability: token_signer_cap, burn_signer_capability, },
        );

        let equipment_info_table = aptos_std::smart_table::new();

        let equipment_info = EquipmentInfo { table: equipment_info_table };
        move_to(&collection_signer, equipment_info);

        let gameData = EquipmentData { max_equipment_level: 50 };

        move_to(&collection_signer, gameData);
    }

    public(friend) entry fun init_upgrade_equipment_capability(
        account: &signer
    ) {
        let caller_address = signer::address_of(account);
        admin::assert_is_admin(caller_address);
        let (object_signer, signer_cap) =
            account::create_resource_account(account, UC_EQUIPMENT_UPGRADE_SEED);

        move_to(&object_signer, EquipmentEnhanceInfo { map: simple_map::new(), });
    }

    public entry fun set_max_weapon_level(
        caller: &signer, new_max_level: u64
    ) acquires EquipmentData {
    ...
    fun get_token_signer(): signer acquires ResourceCapability {
        account::create_signer_with_capability(
            &borrow_global<ResourceCapability>(equipment_collection_address()).capability,
        )
    }

    fun create_equipment_collection(
        signer_resource: &signer, description: String, name: String, uri: String
    ): signer {
        ...
        let collection_capability = EquipmentCollectionCapability { collection_mutator_ref };
    ...
    public(friend) fun mint_equipment(user: &signer, equipment_id: u64) acquires ResourceCapability, EquipmentInfo {
        ...
        let equipment_token =
            create_equipment(
                user,
                equipment_id,
                equipment_info_entry.name,
                equipment_info_entry.description,
                equipment_info_entry.uri,
                equipment_info_entry.equipment_part_id,
                equipment_info_entry.affinity_id,
                equipment_info_entry.grade,
                level,
                equipment_info_entry.hp,
                equipment_info_entry.atk,
                equipment_info_entry.def,
                equipment_info_entry.atk_spd,
                equipment_info_entry.mv_spd,
                equipment_info_entry.growth_hp,
                equipment_info_entry.growth_atk,
                equipment_info_entry.growth_def,
                equipment_info_entry.growth_atk_spd,
                equipment_info_entry.growth_mv_spd,
            );
    ...
    public(friend) entry fun upgrade_equipment(
        from: &signer, equipment_object: Object<EquipmentCapability>, amount: u64
    ) acquires EquipmentCapability, EquipmentData {
        ...
        assert!(
            object::object_address(&collection) == equipment_collection_address(),
            EINVALID_COLLECTION,
        );
        ...
        let current_lvl =
            property_map::read_u64(&equipment_object, &string::utf8(b"LEVEL"));
        ...
        let current_atk_spd =
            property_map::read_u64(&equipment_object, &string::utf8(b"ATK_SPD"));
        let current_mv_spd =
            property_map::read_u64(&equipment_object, &string::utf8(b"MV_SPD"));

        let growth_hp =
            property_map::read_u64(&equipment_object, &string::utf8(b"GROWTH_HP"));
        let growth_atk =
            property_map::read_u64(&equipment_object, &string::utf8(b"GROWTH_ATK"));
        let growth_def =
            property_map::read_u64(&equipment_object, &string::utf8(b"GROWTH_DEF"));
        let growth_atk_spd =
            property_map::read_u64(&equipment_object, &string::utf8(b"GROWTH_ATK_SPD"));
        let growth_mv_spd =
            property_map::read_u64(&equipment_object, &string::utf8(b"GROWTH_MV_SPD"));
        ...
        property_map::update_typed(
            property_mutator_ref,
            &string::utf8(b"HP"),
            current_hp + (amount * growth_hp),
        );
        property_map::update_typed(
            property_mutator_ref,
            &string::utf8(b"ATK"),
            current_atk + (amount * growth_atk),
        );
        property_map::update_typed(
            property_mutator_ref,
            &string::utf8(b"DEF"),
            current_def + (amount * growth_def),
        );
        property_map::update_typed(
            property_mutator_ref,
            &string::utf8(b"ATK_SPD"),
            current_atk_spd + (amount * growth_atk_spd),
        );
        property_map::update_typed(
            property_mutator_ref,
            &string::utf8(b"MV_SPD"),
            current_mv_spd + (amount * growth_mv_spd),
        );
    }

    public(friend) entry fun enhance_equipment(
        from: &signer,
        equipment_object: Object<EquipmentCapability>,
        equipment_object_to_destroy: Object<EquipmentCapability>
    ) acquires EquipmentCapability, EquipmentEnhanceInfo, ResourceCapability {
        assert!(object::is_owner(equipment_object, signer::address_of(from)), ENOT_OWNER);
        assert!(
            object::is_owner(equipment_object_to_destroy, signer::address_of(from)),
            ENOT_OWNER,
        );
        ...
        assert!(
            object::object_address(&collection2) == equipment_collection_address(),
            EINVALID_COLLECTION,
        );
        ...
        let current_lvl =
            property_map::read_u64(&equipment_object, &string::utf8(b"LEVEL"));
        let current_grade =
            property_map::read_u64(&equipment_object, &string::utf8(b"GRADE"));
        let current_grade_2 =
            property_map::read_u64(&equipment_object_to_destroy, &string::utf8(b"GRADE"));

        let equipment_id =
            property_map::read_u64(&equipment_object, &string::utf8(b"EQUIPMENT_ID"));
        let current_equipment_id_2 =
            property_map::read_u64(
                &equipment_object_to_destroy, &string::utf8(b"EQUIPMENT_ID")
            );
        ...
        property_map::update_typed(
            property_mutator_ref, &string::utf8(b"ATK_SPD"), new_atk_spd
        );
        property_map::update_typed(
            property_mutator_ref, &string::utf8(b"MV_SPD"), new_mv_spd
        );
        property_map::update_typed(
            property_mutator_ref, &string::utf8(b"GRADE"), new_grade
        );

        let equipment_upgrade_info =
            borrow_global_mut<EquipmentEnhanceInfo>(equipment_upgrade_info_address());
    ...
    public entry fun destroy_equipment(
        from: &signer, equipment_object: Object<EquipmentCapability>
    ) acquires EquipmentCapability, ResourceCapability {
        assert!(object::is_owner(equipment_object, signer::address_of(from)), ENOT_OWNER);
    ...
    fun burn_equipment(
        from: &signer, equipment_object: Object<EquipmentCapability>
    ) acquires EquipmentCapability {
        assert!(object::is_owner(equipment_object, signer::address_of(from)), ENOT_OWNER);
        let equipment_token =
            move_from<EquipmentCapability>(object::object_address(&equipment_object));
        let EquipmentCapability { mutator_ref, burn_ref, property_mutator_ref } = equipment_token;
        token::burn(burn_ref);
    ...
    public entry fun set_collection_uri(caller: &signer, new_uri: String) acquires EquipmentCollectionCapability {
        let caller_address = signer::address_of(caller);
        admin::assert_is_admin(caller_address);
        let collection_capability =
            borrow_global<EquipmentCollectionCapability>(equipment_collection_address());
    ...
    ) acquires EquipmentCollectionCapability {
        let caller_address = signer::address_of(caller);
        admin::assert_is_admin(caller_address);
        let collection_capability =
            borrow_global<EquipmentCollectionCapability>(equipment_collection_address());
    ...
    public entry fun add_equipment_entry(
        ...
        let equipment_info_table =
            &mut borrow_global_mut<EquipmentInfo>(equipment_collection_address()).table;
    ...
    public entry fun clear_equipment_info_table(account: &signer,) acquires EquipmentInfo {
        admin::assert_is_admin(signer::address_of(account));
    ...
    public entry fun remove_equipment_upgrade_info(
        account: &signer, equipment_id: u64, grade: u64
    ) acquires EquipmentEnhanceInfo {
    ...
    public fun equipment_id_exists(equipment_id: u64): bool acquires EquipmentInfo {
        let equipment_info_table =
            &borrow_global<EquipmentInfo>(equipment_collection_address()).table;
