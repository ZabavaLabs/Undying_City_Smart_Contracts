...
const filePath = './data/daily_spin/rewards.csv';
...
const add_daily_spin_rewards = async () => {
    // Setup the client
    ...
    for (let i = 0; i < rowData.length; i++) {
        transaction = await aptos.transaction.build.simple({
            ...
            data: {
                function: `${account_data.profiles.default.account}::daily_spins::add_result_entry`,
                typeArguments: [],
                functionArguments: [rowData[i][0], rowData[i][1]],
            },
        });
        console.log(`Add Daily Spin Entry ${rowData[i][0]}`);
